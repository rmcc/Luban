...
const createApplication = () => {
    ...
    // Replace the discontinued multiparty with multer, but retain the old
    // req.files syntax to preserve compatibility with the rest of the code.
    // [ { fieldname, originalname, data... } ] becomes { fieldname: { name, data.. } }
    // However... stop intercepting all requests, /api should be enough.
    const upload = multer(settings.middleware.multer);
    app.use(urljoin(settings.route, 'api'), upload.any(), (req, res, next) => {
        // Map the files array to the object type previously generated by multiparty
        // for compatibility
        if (req.files && Array.isArray(req.files)) {
            const fileObject = {};
            req.files.forEach(file => {
                // Use the old property name, too
                file.name = file.originalname;
                fileObject[file.fieldname] = file;
            });
            req.files = fileObject;
        } else {
            req.files = req.files || {};
        }
        next();
